...
function ImportarInvidPage() {
  ...
  useEffect(() => {
    axios.get('/api/admin/invid/estado')
  ...
  const cargarFiltros = async () => {
    ...
    try {
      const res = await axios.get('/api/admin/invid/filtros')
      setFiltros(res.data)
    } catch (e) {
  ...
  if (configurado === null) return (<div><h1>Importar de Invid</h1><div className="card"><p>Cargando...</p></div></div>)

  if (!configurado) return (
    <div>
      <h1>Importar de Invid</h1>
...
INVID_PASSWORD=tu_contraseña`}
        ...
        <p style={{ fontSize: '13px', color: 'var(--color-text-muted)', marginTop: '10px' }}>
          El host y las credenciales te los da Invid (integración API). Después se redeploya solo.
        </p>
      </div>
    </div>
  )
  ...
  return (
    <div>
      ...
      <p style={{ color: 'var(--color-text-muted)', marginBottom: '18px' }}>
        Traé productos del catálogo mayorista de Invid (con precios, stock e imágenes). Se crean/actualizan
        bajo el proveedor <strong>"Invid"</strong>; ajustá su margen y flete en <Link to="/admin/proveedores" style={{ color: 'var(--color-accent)', fontWeight: 600 }}>Proveedores</Link>.
      </p>
...
export default ImportarInvidPage
