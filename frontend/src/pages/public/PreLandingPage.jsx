    ...
    <section className="prelanding-content"><div className="prelanding-copy prelanding-reveal prelanding-reveal-two">
      ...
      <ol className="prelanding-steps">
        {pasoVisible === 1 && <li className={pasoUno ? 'completo' : ''}><span className="prelanding-step-number">1</span><div><b>{pasoUno ? 'Email activado.' : 'Registrate con tu número de WhatsApp.'}</b><small>{pasoUno ? 'Tu cuenta ya está confirmada.' : 'Te enviaremos un botón de activación por email.'}</small>
          ...
          {necesitaRegistro ? <Link className="prelanding-action primary" to="/registro">Comenzar registro →</Link> : pasoUno ? <span className="prelanding-done"><IconCheck /> Listo</span> : null}</li>}
        ...
        {pasoVisible === 3 && <li className={pasoTres ? 'completo' : (!pasoDos ? 'bloqueado' : (instagramPendiente ? 'pendiente' : ''))}><span className="prelanding-step-number">3</span><div><b>{pasoTres ? 'Instagram validado.' : instagramPendiente ? 'Estamos validando tu Instagram.' : 'Seguinos y etiquetá a 3 amigos en Instagram.'}</b><small>{pasoTres ? 'Tu participación ya quedó validada.' : instagramPendiente ? `Ya registramos tu aviso. Verificaremos ${estado?.instagramUsuario || 'tu usuario'} y la consigna del sorteo.` : 'Hacelo en el posteo del sorteo para dejar registrada tu participación.'}</small></div>
          {pasoTres ? <span className="prelanding-done"><IconCheck /> Listo</span> : <div className="prelanding-actions"><a className={`prelanding-action instagram${instagramPendiente ? ' disabled' : ''}`} href={pasoDos && !instagramPendiente ? instagramUrl : undefined} target="_blank" rel="noreferrer" onClick={e => (!pasoDos || instagramPendiente) && e.preventDefault()}>Ir a Instagram</a><button className="prelanding-action outline" disabled={!pasoDos || instagramPendiente || guardando} onClick={() => completar(3)}>Ya lo hice</button></div>}</li>}
      </ol>
      {mensaje && <p className="prelanding-message">{mensaje}</p>}
      {pasoTres && !modalCerrado && <div className="prelanding-modal-backdrop" role="presentation"><section className="prelanding-modal" role="dialog" aria-modal="true" aria-labelledby="prelanding-modal-title"><span className="prelanding-modal-check"><IconCheck /></span><span className="prelanding-modal-label">PREINSCRIPCIÓN CONFIRMADA</span><h2 id="prelanding-modal-title">¡Ya estás participando!</h2><p>Próximamente tendrás acceso al gran catálogo tecnológico y con precios increíbles.</p><p><strong>Ah!</strong> Y también tendrás regalos para cada cumpleaños tuyo.</p><button className="prelanding-modal-close" onClick={() => setModalCerrado(true)}>Entendido</button><span className="prelanding-modal-note">Gracias por sumarte a FuturaTecno.</span></section></div>}
      {estado?.codigoSorteo && <div className="prelanding-ticket"><span>Tu código de sorteo</span><code>{estado.codigoSorteo}</code>{estado.chancesSorteo > 1 && <b><IconStar /> Doble chance: tu participación se incluirá dos veces.</b>}<small>También te lo enviamos por email. Guardalo hasta el sorteo.</small></div>}
      <div className="prelanding-reminder"><span><IconCheck /></span><span><b>¡Importante!</b> Registrarte y completar los tres pasos es condición para participar. <strong>Sin obligación de compra.</strong> <Link to="/bases-y-condiciones">Ver Bases y Condiciones</Link>.</span></div>
