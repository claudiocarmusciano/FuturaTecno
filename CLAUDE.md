...
- **Los timeouts del cliente HTTP NO alcanzan: falta el DNS.** `setConnectTimeout` y `setReadTimeout` cubren la conexión y la lectura, pero la resolución de nombres en Java **no tiene límite** y se cuelga para siempre. El 2026-09-14 una llamada a DeepSeek desde Railway quedó trabada **más de 7 minutos** sin que saltara el read timeout de 180 s ni se escribiera un solo error — el navegador se rendía y el admin veía una pantalla muda. Por eso `GenerarListadoService.generar()` corre la llamada en un hilo aparte con un **tope duro de 150 s** (`Future.get`), menos que los 240 s del navegador. `cancel(true)` no rescata al hilo trabado: se lo abandona a propósito, es preferible perder un hilo del pool a perder la pantalla. **Toda llamada saliente nueva que pueda colgar una request de usuario necesita el mismo tratamiento**, no solo timeouts de socket.
